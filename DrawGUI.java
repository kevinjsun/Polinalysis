 ...
 *  @author Kevin Wayne
 */
public final class DrawGUI implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static DrawGUI std = new DrawGUI();
    ...
    *************************************************************************/
    ...
    private static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = DrawGUI.class.getResource(filename);
    ...
    *************************************************************************/
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(DrawGUI.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            DrawGUI.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    *************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = DrawGUI.userX(e.getX());
            mouseY = DrawGUI.userY(e.getY());
            mousePressed = true;
        }
    }
    ...
    *************************************************************************/
    ...
    public static void main(String[] args) {
        DrawGUI.square(.2, .8, .1);
        DrawGUI.filledSquare(.8, .8, .2);
        DrawGUI.circle(.8, .2, .2);

        DrawGUI.setPenColor(DrawGUI.BOOK_RED);
        DrawGUI.setPenRadius(.02);
        DrawGUI.arc(.8, .2, .1, 200, 45);

        // draw a blue diamond
        DrawGUI.setPenRadius();
        DrawGUI.setPenColor(DrawGUI.BOOK_BLUE);
        double[] x = { .1, .2, .3, .2 };
        double[] y = { .2, .3, .2, .1 };
        DrawGUI.filledPolygon(x, y);

        // text
        DrawGUI.setPenColor(DrawGUI.BLACK);
        DrawGUI.text(0.2, 0.5, "black text");
        DrawGUI.setPenColor(DrawGUI.WHITE);
        DrawGUI.text(0.8, 0.8, "white text");
    }
    
    public static void picture(double x, double y, Image i, double w, double h) {
        Image image = i;
        double xs = scaleX(x);
        double ys = scaleY(y);
        double ws = factorX(w);
